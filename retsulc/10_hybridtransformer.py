"""
End-to-end pipeline with FIELD-WISE HYBRID TRANSFORMER encoder

Features:
- Per-field small CNN encoder (captures local char patterns)
- Transformer encoder *over field vectors* (cross-field attention)
...
- DBSCAN clustering (automatic simple hyperparameter search)
- NO greedy matching / NO postprocessing — rely on the model to place amount-balanced items together
- Optional pairwise evaluation (precision/recall/F1) using MatchGroupId

Notes:
- Replace the example `df` creation with `pd.read_csv(...)` when running on full dataset.
- Tune hyperparameters at the top (EPOCHS, BATCH_SIZE, PROJ_DIM, DBSCAN grid).
- Requires: torch, sklearn, scipy, numpy, pandas
"""
...
# DBSCAN hyper-search grid (tune for your dataset)
DBSCAN_EPS = [0.2, 0.5, 0.8, 1.0]
DBSCAN_MIN_SAMPLES = [2, 3, 5]

# ----------------------------
# 0) Load data (replace with your CSV)
# ----------------------------
# Small example — replace with real dataset
data = {
...
# ----------------------------
# 1) Build character vocabulary from chosen fields
# ----------------------------
all_text = ""
for f in FIELDS:
    if f in df.columns:
...
# ----------------------------
# 2) Contrastive dataset (MatchGroupId positives)
# ----------------------------
class MatchGroupContrastiveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while pos_idx == anchor_idx:
            pos_idx = random.choice(members)
        x1 = np.array(build_row_seq(self.df.loc[anchor_idx]), dtype=np.int64)
        x2 = np.array(build_row_seq(self.df.loc[pos_idx]), dtype=np.int64)
        a1 = float(self.df.loc[anchor_idx, "SignedAmount"])
        a2 = float(self.df.loc[pos_idx, "SignedAmount"])
        return x1, x2, a1, a2
...
# ----------------------------
# 3) Model: field-specific CNN encoders -> Transformer (over fields) -> combine with amount
# ----------------------------
...
class FieldWiseHybridTransformer(nn.Module):
    def __init__(self, vocab_size, seq_per_field=SEQ_LEN, num_fields=len(FIELDS),
        ...
        # signed amount projection into same dim
    ...
    def forward(self, seq_batch, amount_batch):
        ...
        combined = pooled + amt_proj                # incorporate amount signal
        z = self.proj(combined)                     # (batch, proj_dim)
        z = F.normalize(z, p=2, dim=1)
        return z

model = FieldWiseHybridTransformer(VOCAB_SIZE, seq_per_field=SEQ_LEN, num_fields=len(FIELDS)).to(DEVICE)

# ----------------------------
# 4) NT-Xent / InfoNCE loss for positive pairs
# ----------------------------
...
# ----------------------------
# 5) Scale amounts for training (robust standardization)
# ----------------------------
...
# ----------------------------
# 6) Contrastive training loop (MatchGroupId positives)
# ----------------------------
model.train()
...
# ----------------------------
# 7) Produce embeddings for all rows (inference)
# ----------------------------
model.eval()
with torch.no_grad():
...
# ----------------------------
# 8) Build feature matrix for clustering (embedding +/- optional categorical features)
# ----------------------------
...
# ----------------------------
# 9) DBSCAN hyperparameter search (eps, min_samples)
# ----------------------------
best_score = -1.0
best_labels = None
best_params = None
...
df.to_csv("clusters_fieldwise_transformer_contrastive_dbscan.csv", index=False)
print("Done.")
